...
def naiveSoftmaxLossAndGradient(
    ...
    val = np.dot(outsideVectors, centerWordVec)
    prob = softmax(val)
...
def negSamplingLossAndGradient(
    ...
    # get vectors
    vec_out = outsideVectors[outsideWordIdx]
    vec_neg = outsideVectors[negSampleWordIndices]

    # calculate sigmoid and loss
    y_hat  = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    y_k_hat = sigmoid(-vec_neg.dot(centerWordVec))
    loss = - (np.log(y_hat) + np.sum(np.log(y_k_hat)) )
    
    gradCenterVec = (y_hat - 1) * vec_out + np.sum((1 - y_k_hat)[:, np.newaxis] * vec_neg, axis = 0)
    gradOutsideVecs[outsideWordIdx] = (y_hat - 1) * centerWordVec
    
    # second term calculation
    for i, val in enumerate(negSampleWordIndices):
        gradOutsideVecs[val] += (1 - y_k_hat[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    outsideWordIdx = [word2Ind[i] for i in outsideWords]
    for idx in outsideWordIdx:
        current_loss, current_centerGradient, current_outGradient = \
            word2vecLossAndGradient(centerWordVec, idx, outsideVectors, dataset)
        loss += current_loss
        gradCenterVecs[centerWordIdx] += current_centerGradient
        gradOutsideVectors += current_outGradient
