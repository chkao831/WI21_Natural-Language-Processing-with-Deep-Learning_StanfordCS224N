...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Apply the `pack_padded_sequence` function to X
        X = pack_padded_sequence(X, lengths=source_lengths)
        # Apply the `pad_packed_sequence` function to enc_hiddens
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            # Squeeze Y_t into a tensor of dimension (b, e).
            squeezed = torch.squeeze(Y_t) # (b, e)
            # concatenating Y_t with o_prev on their last dimension
            Ybar_t = torch.cat((squeezed, o_prev), dim=1) # (b, e + h)
            # Use the step function to compute the the Decoder's next (cell, state) & the new combined output o_t
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            # Append o_t to combined_outputs
            combined_outputs.append(o_t)
            # Update o_prev to the new o_t
            o_prev = o_t
        # 4. Use torch.stack to convert combined_outputs from a list length tgt_len
        # of tensors shape (b, h), to a single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # to obtain attention output vector, a_t
