...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure random truncation: every possible length from 4 to int(self.block_size*7/8)
        # has a chance of being picked. 
        rand_trunc = random.randint(min(4,len(dat)),
                                    min(len(dat),
                                    int(self.block_size*7/8)))
        dat = dat[0:rand_trunc] # truncated

        # Break the truncated document into three substrings: [prefix] [masked_content] [suffix]
        # The length of [masked_content] should be random at around 1/4 the len of the truncated doc.
        rand_trunc_masked_content = int(random.uniform(1/8,3/8)*len(dat))
        div_start = random.randint(0,len(dat)-rand_trunc_masked_content)
        
        prefix = dat[0:div_start]
        masked_content = dat[div_start:(div_start+rand_trunc_masked_content)]
        suffix = dat[(div_start+rand_trunc_masked_content):]

        # Rearrange to [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        # s.t. the resulting str masked_string contains 
        # MASK_CHAR as defined masking character and [pads] as string of repeated PAD_CHAR characters
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content +\
                        self.PAD_CHAR*(self.block_size - len(dat) - 2)

        # Take the input string to be masked_string[:-1], and the output string to be masked_string[1:]
        input_str = masked_string[:-1]
        output_str = masked_string[1:]

        # Encode the resulting input and output strings as Long tensors and return the resulting data pt.
