...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(max_epochs=650, 
                                  ...
                                  final_tokens=200*len(pretrain_dataset)*block_size,
                                  num_workers=4)
    # Pretrain the model on this corpus
    pretrain_corpus = open(args.pretrain_corpus_path, encoding="utf8").read()
    dat = dataset.CharCorruptionDataset(pretrain_corpus, block_size)
    trainer = trainer.Trainer(model, dat, None, tconf)
    trainer.train()
    ...
    if args.reading_params_path: # if reading_params_path is specified, load 
