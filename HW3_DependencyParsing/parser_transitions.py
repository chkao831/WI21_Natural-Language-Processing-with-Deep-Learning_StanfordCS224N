...
def minibatch_parse(sentences, model, batch_size):
    ...
    # capture number of sentences
    num_sentences = len(sentences)
    # create list of partial parses using list comprehension
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    # create shallow copy that refers to partial_parses
    unfinished_parses = partial_parses[:] 

    # while unfinished_parses is not empty, do
    while unfinished_parses:
        # Take the first batch_size parses as minibatch
        parser = unfinished_parses[:batch_size]
        # Use the model to predict the next transition for each partial parse in minibatch
        transitions = model.predict(parser)
        # On each partial parse in the minibatch
        for p, predicted_transition in zip(parser, transitions):
            # Perform a parse step with its predicted transition
            p.parse([predicted_transition])
            # Remove the completed parses
